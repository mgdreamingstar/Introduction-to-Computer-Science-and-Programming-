...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while sum(copy_hand.values()) != 0:
        # Display the hand
        print 'Current Hand:', displayHand(copy_hand)
        # Ask user for input
        enter = raw_input('Enter word, or a "." to indicate that you are finished:')
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isValidWord(enter,copy_hand,wordList):
                # Reject invalid word (print a message followed by a blank line)
                print 'Invalid word, please try again.\n'
            # Otherwise (the word is valid):
            else:
                # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                score += getWordScore(enter,n)
                print enter, 'earned', getWordScore(enter,n), 'points', ', and the total score is', score, '\n'
                # Update the hand
                copy_hand = updateHand(copy_hand,enter)

    # Game is over (user entered a '.' or ran out of letters), so tell user the total score
    print 'Goodbye! Your total score is', score
